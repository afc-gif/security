...
<div class="container mx-auto py-6">
    <h1 class="text-3xl font-bold mb-6">Create Solution Category</h1>

    <form action="{{ route('admin.solutions.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input type="text" name="icon" id="icon" class="w-full px-4 py-2 border rounded @error('icon') border-red-500 @enderror" 
                   value="{{ old('icon') }}" placeholder="e.g. 📹" maxlength="10" required>
            @error('icon')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
