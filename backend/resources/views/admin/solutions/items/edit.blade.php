    ...
    <form action="{{ route('admin.solutions.items.update', [$solution, $item]) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="stock" id="stock" min="0" required 
                   ...
                   value="{{ old('stock', $item->stock) }}" placeholder="Enter stock quantity">
            <span style="color: #16a34a; font-size: 12px; font-weight: bold; display: block; margin-top: 4px;">⭐ Update stock quantity here - triggers alerts at 2 or 0</span>
            @error('stock')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
