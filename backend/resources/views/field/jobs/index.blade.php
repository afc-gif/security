...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 shadow-xs">
        <span class="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">Jobs Board</span>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">Field Jobs</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Claim available category items and submit active job reports.</p>
    </div>

    <!-- Available Jobs -->
    <div class="space-y-3">
        <h2 class="text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider">Available Jobs</h2>
                    ...
                    <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm space-y-3">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 font-medium mt-0.5">{{ $job->jobRequest?->client?->client_name ?? 'Client name unavailable' }}</p>
                            </div>
                            <span class="px-2 py-0.5 rounded-md text-[9px] font-extrabold uppercase bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-450 border border-emerald-100 dark:border-emerald-900/50 whitespace-nowrap">
                                Open
                            </span>
                        </div>
                        ...
                        <div class="flex items-center justify-between text-[10px] text-slate-500 border-t border-slate-50 dark:border-slate-850 pt-2.5">
                            ...
                            <form method="POST" action="{{ route('field.jobs.claim', $job) }}" onsubmit="return confirm('Do you want to claim this job?');">
                                @csrf
                                <button type="submit" class="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-extrabold">
                                    Claim Job &rarr;
                                </button>
                            </form>
                        </div>
    ...
    <!-- My Claims -->
    <div class="space-y-3">
        <h2 class="text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider">My Claimed Jobs</h2>
                    ...
                    <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm space-y-3">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <span class="px-2 py-0.5 rounded-md text-[9px] font-extrabold uppercase whitespace-nowrap
                                {{ $isOverdue ? 'bg-rose-50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-450 border border-rose-100 dark:border-rose-900/50' : 'bg-indigo-50 dark:bg-indigo-950/20 text-indigo-700 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/50' }}">
                                {{ str_replace('_', ' ', \Illuminate\Support\Str::title($displayStatus)) }}
                            </span>
                        </div>

                        <div class="flex flex-wrap gap-1.5">
                            <span class="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-355 rounded-lg text-[9px] font-bold">
                                {{ $job->serviceCategory?->name ?? 'Service Category' }}
                            </span>
                        </div>

                        <div class="flex items-center justify-between text-[10px] text-slate-500 border-t border-slate-50 dark:border-slate-850 pt-2.5">
                            <span class="{{ $isOverdue ? 'text-rose-600 dark:text-rose-450 font-semibold' : 'text-slate-400 dark:text-slate-500' }}">
                            ...
                            <a href="{{ route('field.jobs.show', $job) }}" class="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-extrabold">
                                Open &rarr;
                            </a>
                        </div>
                    </div>
