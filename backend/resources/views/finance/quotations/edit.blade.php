...
@section('title', 'Edit Quotation ' . $quotation->quotation_number . ' | ARTSCI Finance')
...
<div class="finance-page">
    <div class="finance-wrap">
        ...
        <a href="{{ route('finance.quotations.show', $quotation) }}" class="finance-back-link"><- Back to Quotation Details</a>

        <div class="finance-header mb-6">
            <h1 class="finance-title">Edit Quotation {{ $quotation->quotation_number }}</h1>
            <p class="finance-subtitle">Update line items, pricing, notes, and terms.</p>
        </div>
        ...
        <form method="POST" action="{{ route('finance.quotations.update', $quotation) }}" id="quotationForm" class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="finance-form-group">
                    ...
                    <input id="quotation_date" name="quotation_date" value="{{ old('quotation_date', $quotation->quotation_date?->format('Y-m-d')) }}" type="date" required class="finance-form-input">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <div class="space-y-4">
                    <div class="finance-form-group">
                        ...
                        <textarea id="notes" name="notes" rows="3" class="finance-form-input">{{ old('notes', $quotation->notes) }}</textarea>
                    </div>

                    <div class="finance-form-group">
                        <label for="terms" class="finance-form-label">Terms & Conditions (Optional)</label>
                        <textarea id="terms" name="terms" rows="3" class="finance-form-input">{{ old('terms', $quotation->terms) }}</textarea>
                    </div>
                </div>
                ...
                <div class="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3">
                    ...
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <input id="discount_amount" name="discount_amount" value="{{ old('discount_amount', $quotation->discount_amount) }}" type="number" min="0" step="0.01" class="finance-form-input text-right w-36">
                    </div>

                    <div class="flex items-center justify-between gap-4">
                        <label for="tax_amount" class="text-sm font-medium text-slate-600 whitespace-nowrap">Tax (₦):</label>
                        <input id="tax_amount" name="tax_amount" value="{{ old('tax_amount', $quotation->tax_amount) }}" type="number" min="0" step="0.01" class="finance-form-input text-right w-36">
                    </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
                <a href="{{ route('finance.quotations.show', $quotation) }}" class="finance-btn finance-btn-secondary">
                    Cancel
                </a>
...
(function() {
    ...
    const initialItems = @json($quotation->items);
    ...
    function createRow(description = '', qty = 1, price = 0) {
        ...
        tr.innerHTML = `
            ...
            <td class="p-3">
                <input type="text" name="items[${index}][description]" value="${description.replace(/"/g, '&quot;')}" required placeholder="Item description / service" class="finance-form-input item-desc">
            </td>
            <td class="p-3">
    ...
    if (initialItems && initialItems.length > 0) {
        initialItems.forEach(item => createRow(item.description, item.quantity, item.unit_price));
    } else {
        createRow();
    }
})();
